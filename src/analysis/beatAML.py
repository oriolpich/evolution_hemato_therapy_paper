...
def get_orders(subset_drivers, sample_taml_beat):
    ...
    wanted_genes = [s for s in sorted_genes if dic_counts[s] > 1]
...
def get_samples_data(mutations_beatAML, clinical_data, drivers_AML):
    ...
    sample_taml_beat = \
    df_metadata_beat[(df_metadata_beat['SPECIFIC_DIAGNOSIS_AT_INCLUSION'] == 'Therapy-related myeloid neoplasms')][
        'SAMPLE'].unique()

    muts_cbio = muts_cbioport[
        ['Hugo_Symbol', 'Chromosome', 'Start_Position', 'Consequence', 'Variant_Type', 'short_aa_change',
...
def do_barplot(mutations_beatAML, clinical_data, drivers_AML, outpath):
    subset_drivers, sample_taml_beat, muts_cbio, driver_AML = get_samples_data(mutations_beatAML, clinical_data,
                                                                               drivers_AML)
...
def do_heatmap(mutations_beatAML, clinical_data, drivers_AML, outpath):
    ...
    final_ord, primary, second, number_drivers, n_drivers_sample, matrix_driver_genes = get_type_alts(subset_drivers,
                                                                                                      final_ord,
                                                                                                      primary, second,
                                                                                                      wanted_genes,
                                                                                                      dic_sample)
...
def odds_ratio_tAMLs(mutations_beatAML, clinical_data, drivers_AML, outpath):
    ...
    for aml, data in muts_cbio.groupby(by='tAML'):
        for gene in driver_AML:
            ...
            if gene == "DNMT3A":
                set_dnmt = len(data[(data['Hugo_Symbol'] == gene) & (data["short_aa_change"].str.contains("R882"))][
                                   'SAMPLE'].unique())
                dic_res[aml]["DNMT3A p.R882"] = set_dnmt
                set_dnmt = len(
                    data[(data['Hugo_Symbol'] == gene) & (~data["short_aa_change"].str.contains("R882", na=False))][
                        'SAMPLE'].unique())
    ...
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    plt.vlines(0, 0, len(res_reg), color='grey')
...
# FIXME: NOT USED
def CH_vs_tAML():
    # treated_full = coding_variants_merged[coding_variants_merged['TREAT']=='YES']
    ...
    for gene in unique_set:
        ...
        # muts_CH = len(set_muts_CH[(set_muts_CH['GENE']==gene)&(set_muts_CH['MUTECT']=='YES')]['SAMPLE'].unique())
        ...
        if gene == "DNMT3A":
            ...
            muts_CH = len(
                set_muts_CH[(set_muts_CH['SYMBOL'] == gene) & (set_muts_CH["short_aa_change"].str.contains('882R'))][
                    'SAMPLE'].unique())
            muts_tAML = len(set_muts_taml[(set_muts_taml['Hugo_Symbol'] == gene) & (
                set_muts_taml["short_aa_change"].str.contains('R882'))]['SAMPLE'].unique())
    ...
    for ix, (i, row) in enumerate(res.sort_values(by='tAML', ascending=False).head(40).iterrows()):
        ...
        plt.scatter(np.log2(odds), ix, color=color, edgecolor=edge)

        plt.plot([np.log2(cmin), np.log2(cmax)], [ix, ix], color=color)
        xlab.append(i)

        print(i, odds, pval)
    plt.vlines(0, 0, 30, ls='--')
    plt.yticks(np.arange(30), xlab)
    plt.xlim()
    plt.xlabel('odds-ratio')
    plt.title('CH-treated/t-AML')
